...
export class AdvancedChannelSelectMenuBuilder {
	...
	public async build(): Promise<AdvancedChannelSelectMenu> {
		...
		const startIndex = Math.max(
			this.calculatePageStartIndex(this.page, maxPages) - Number(this.options.allowUnset),
			0
		);
		...
		// -> Add delete button
		if (this.options.allowUnset && this.page === 1) {
			options.splice(0, 0, this.getDeleteOption());
		}
	...
	private calculatePagesForChannelAmount(amount: number): number {
		if (amount <= 25 + Number(this.options.allowUnset)) {
			// 25 items fit on one page, no next page option needed
			return 1;
		} else if (amount <= 24 * 2 + Number(this.options.allowUnset)) {
			// 48 items fit on 2 pages, only one next/prev page option needed per page
			return 2;
		} else {
	...
	private getDeleteOption(): StringSelectMenuOptionBuilder {
		return new StringSelectMenuOptionBuilder()
			.setValue(EAdvancedChannelSelectReturnValue.UNSET)
			.setLabel(EAdvancedChannelSelectReturnValue.UNSET)
			.setEmoji(EMOJIS.UNSET);
	}
